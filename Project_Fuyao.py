'''
根据项目描述，我们需要实现 10 个功能，具体如下
（1）构建一个字典，叫 Fuyao_Actor_Profile,包括演员名字，饰演角色，配音演员
（2）打印出杨幂扮演的角色是谁？
（3）创建一个备份字典 Copy_Fuyao
（4）在演员表中删除阮经天
（5）更替为陈晓
（6）增加新的角色
...
（9）重新创建一个新的字典
（10）新字典存放以下信息：
 扶摇的名字、喜欢她的角色有（长孙无极、战北野、小七）、去过的国家（太渊、天
权、天煞、璇玑）
'''

# 构建一个字典，叫 Fuyao_Actor_Profile,包括演员名字，饰演角色，配音演员
Fuyao_Actor_Profile = {
 '杨幂': {
 'Role_Name': '扶摇',
 'Voice_Actor': '王潇倩'
